...
struct order {
  ...
  order(order::type in_ordertype, sp<stock> in_stock, int in_quantity,
...
struct orderBook {
  ...
  struct greater {
    bool operator()(sp<order> a, sp<order> b) {
  ...
  v<sp<order>> execute_trades(mtx &in_ordmtx) {
    ...
    for (auto &[stock_name, buy_queue] : m_buy_orders) {
      ...
      while (not buy_queue.empty() and not sell_queue.empty()) {
        ...
        bool buyer_is_offering_enough =
            buy_order->m_price >= sell_order->m_price;
        bool market_order_fast =
            buy_order->m_pref == order::preference::market or
...
struct portfolio {
  ...
  double register_stock_sold(sp<stock> in_stock, int in_quantity,
                             double in_cash) {
    m_holdings[in_stock] -= in_quantity;
...
struct trader {
  ...
  void make_order(order::type in_type, sp<stock> in_stock, int in_quantity,
                  order::preference in_pref, mtx &in_trademtx) {
    sp<order> order_ =
        mksh<order>(in_type, in_stock, in_quantity, m_id, in_pref);
  ...
  void do_random_action(v<sp<stock>> in_stocks, mtx &in_trademtx) {
    ...
    if (has_holdings and random_action < 0.22) {
      ...
      m_available_cash = m_porfolio.register_stock_sold(stock, sell_quantity,
                                                        m_available_cash);
    }
    ///@note Buy on these
    ///
...
struct market {
  ...
  void run() {
    ...
    m_traders_thread = std::thread([&] {
      ...
      while (m_running) {
        if (not m_paused) {
          m_traders[trader_dist(m_rang)]->do_random_action(m_stocks,
                                                           m_trade_mtx);
        }
      }
    });
