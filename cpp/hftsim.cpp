...
void hft::stock::edit_price(double in_amount) {
  m_price = m_price * (1 + in_amount);
}
...
hft::stock::stock(s in_name, double in_price)
    : m_name(in_name), m_price(in_price) {
  m_factors = 1.0;
}
hft::order::order(order::type in_ordertype, sp<stock> in_stock, int in_quantity,
                  int in_id, order::preference in_pref) {
  m_type = in_ordertype;
  m_stock = in_stock;
...
double hft::portfolio::make_change(sp<order> in_order,
                                   double in_available_cash) {
  if (in_order->m_type == order::type::buy) {
...
void hft::portfolio::register_stock_bought(sp<stock> in_stock,
                                           int in_quantity) {
  m_holdings[in_stock] += in_quantity;
}
double hft::portfolio::register_stock_sold(sp<stock> in_stock, int in_quantity,
                                           double in_cash) {
  m_holdings[in_stock] -= in_quantity;
...
void hft::trader::make_order(order::type in_type, sp<stock> in_stock,
                             int in_quantity, order::preference in_pref,
                             mtx &in_trademtx) {
...
void hft::market::run() {
  ...
  m_event_thread = std::thread([&] {
    ...
    while (m_running) {
      if (not m_paused) {
        ...
        if (dist(m_rang)) {
          apply_market_impact(generate_random_market_event(), m_event_mtx);
        } else {
  ...
  m_traders_thread = std::thread([&] {
    ...
    while (m_running) {
      if (not m_paused) {
        m_traders[trader_dist(m_rang)]->do_random_action(m_stocks, m_trade_mtx);
      }
    }
  });
}
...
void hft::market::stop() { m_running = false; }
void hft::market::pause() { m_paused = true; }
void hft::market::resume() { m_paused = false; }
...
hft::event hft::market::generate_random_market_event() {
  ...
  static const std::vector<int> weights = {
      ...
      5, 3, 2, 3,  2,  1, 2,  3,  1,  2, 2,  3,  2,  1, 1, 1,
      1, 2, 2, 10, 10, 5, 10, 10, 5,  5, 10, 10, 10, 1, 20};
...
void hft::market::apply_market_impact(event in_event, mtx &in_event_mtx) {
  ...
  case event::sanctions_announced:
    applyFactorToAll(factors *
                     (in_event == event::fiscal_stimulus ? 1.2 : 0.8));
    break;
